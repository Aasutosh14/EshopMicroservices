...
{
    public class UpdateOrderHandler(IApplicationDBContext dbContext) : ICommandHandler<UpdateOrderCommand, CreateOrderResult>
    {
        ...
        private Order CreateNewOrder(OrderDto order)
        {
            ...
            var newOrder = Order.Create(
                ...
                payment: order.Payment == null ? null : Payment.Of(order.Payment.CardNumber, order.Payment.CardName, order.Payment.CardHolderName, order.Payment.Expiration, order.Payment.Cvv, order.Payment.PaymentMethod)
                );
